...
session_name="g1_sprint_e2b0b2"
root="evaluations/Unitree-G1-Sprint-E2B0B2-Yaw-Focus/seed42"
source_checkpoint="logs/rsl_rl/g1_velocity/2026-08-24_02-33-07_sprint_e2b0b_reward_fix_4096_100/model_99.pt"
python_bin="/data/users/yanghao/envs/humanoid/unitree_rl_mjlab-py311/bin/python"
runtime=(
  CUDA_VISIBLE_DEVICES=5 MUJOCO_GL=egl MUJOCO_EGL_DEVICE_ID=0
...
shopt -s nullglob
run_dirs=(logs/rsl_rl/g1_velocity/*_sprint_e2b0b2_yaw_focus_4096_100)
if (( ${#run_dirs[@]} == 0 )); then
  echo "Could not locate B0-B2 run" >&2
  exit 1
fi
run_dir="${run_dirs[-1]}"
common=(
  Unitree-G1-Sprint-E2B0B2-Yaw-Focus
...
for spec in \
  "straight:0.0:0.0" "lat_pos:0.3:0.0" "lat_neg:-0.3:0.0" \
  "yaw_pos_015:0.0:0.15" "yaw_neg_015:0.0:-0.15"; do
  name="${spec%%:*}"
  rest="${spec#*:}"
  lateral="${rest%%:*}"
  yaw="${rest##*:}"
  env "${runtime[@]}" "$python_bin" scripts/evaluate.py "${common[@]}" \
    ...
    --command-yaw-rate-radps "$yaw" --output-dir "$root/$name"
done
"$python_bin" scripts/select_sprint_e2b0_checkpoint.py \
  ...
  --lat-pos "$root/lat_pos/results.json" --lat-neg "$root/lat_neg/results.json" \
  --yaw-pos-015 "$root/yaw_pos_015/results.json" \
  --yaw-neg-015 "$root/yaw_neg_015/results.json" --output "$root/decision.json"
